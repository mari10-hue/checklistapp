...
function submitChecklist() {
  ...
  // Validation spécifique du Plan d'Action (Maintenant Facultatif)
  const storageField = `action_plan_${currentChecklistType.toLowerCase()}`;
  const actionPlanData = currentSessionAnswers[storageField];
  
  if (hasNonConformity && actionPlanData && Array.isArray(actionPlanData) && actionPlanData.length > 0) {
      // Vérifier que chaque ligne a des informations de base seulement SI l'utilisateur a ajouté des lignes
      ...
      if (rowIncomplete) {
          isValid = false;
          missingItems.push(`- Le Plan d'Action commencé contient des lignes incomplètes (Action, Pilote ou Date manquante). Vous pouvez supprimer la ligne si elle n'est pas nécessaire.`);
      }
  }
...
function setupEventListeners() {
  ...
  if (btnBackChecklist) {
    btnBackChecklist.addEventListener('click', () => {
      editingChecklistId = null; // Clear edit context
  ...
  if(btnClearHistory) {
    btnClearHistory.addEventListener('click', () => {
      if(confirm("⚠ Êtes-vous sûr de vouloir supprimer définitivement tout l'historique de toutes les sessions ? (Irreversible)")) {
        localStorage.removeItem('SQCDMN_Responses'); // FIX: changed from SQCDMN_Sessions
...
if (pwaInstallBtn) {
  pwaInstallBtn.addEventListener('click', async () => {
    if (typeof deferredPrompt !== 'undefined' && deferredPrompt) {
      ...
      if (isIos()) {
        alert("Sur iPhone/iPad, Apple bloque le bouton direct. Veuillez appuyer sur l'icône de partage (Carré avec flèche) puis sur 'Sur l'écran d'accueil'.");
      } else {
        alert("Impossible d'installer automatiquement (vérifiez que vous utilisez Google Chrome et une connexion sécurisée). Utilisez le menu avec les 3 petits points en haut à droite, puis choisissez 'Ajouter à l'écran d'accueil'.");
      }
    }
  });
}
...
window.forceShowPwaModal = () => {
  const pwaModalToOpen = document.getElementById('pwa-install-modal');
  const btnPWAInstallInner = document.getElementById('pwa-install-btn');
  const instructionsPwa = document.getElementById('pwa-ios-instructions');
  
  if (pwaModalToOpen) {
    pwaModalToOpen.style.display = 'flex';
    if(btnPWAInstallInner) btnPWAInstallInner.style.display = 'block'; // ALWAYS SHOW BUTTON
    
    // Modify instructions to act as a tip rather than replacing the button
    if(instructionsPwa) {
      if (isIos()) {
        instructionsPwa.style.display = 'block';
        instructionsPwa.innerHTML = `<em>Astuce iPhone :</em> Apple bloque le bouton direct. Appuyez sur le menu de Partage, puis sur <strong>"Sur l'écran d'accueil"</strong>.`;
      } else {
        instructionsPwa.style.display = 'none';
      }
    }
  }
};
...
if (btnManualPwa) {
  btnManualPwa.addEventListener('click', window.forceShowPwaModal);
}
